...
async def netperf(
    ...
    netperf_client = detail.netperf_shell.NetperfShell(shell=demo1.shell)
    ...
    try:
        ...
        await asyncio.gather(
            demo0.shell.run_async("netserver -4"),
            netperf_client.run_async(
                f"-H {server_ip} -l {test_length} -t TCP_RR -- -D",
                delay=2,
                cwd=script_path(),
            ),
            monitor(log=log),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test_length", type=int, help="The number of packet to receive", default=10
    )
