...
def main():
    ...
    results = {"Average Throughput / MB/s": [],
               "Packet Loss / %": [], "Rely": [],
               "Server Latency": [], "Client Latency": [],
               "Server Throughput": [], "Transfers": []}
    files = 0
    while path.exists(f"./samples/tcp/throughput{args.throughput}_file{files}.json"):
        files += 1
    for i in range(files):
        with open(f"./samples/tcp/throughput{args.throughput}_file{i}.json", "r") as f:
            temp = json.load(f)
            print(temp)
            results["Average Throughput / MB/s"].append(
                temp["Mean Throughput / MB/s"])
    ...
    rely_files = 0
    while path.exists(f"./samples/rely/rely_throughput{args.throughput}_file{rely_files}.json"):
        rely_files += 1
    for i in range(rely_files):
        with open(f"./samples/rely/rely_throughput{args.throughput}_file{i}.json", "r") as f:
            temp = json.load(f)
            print(temp)
            results["Average Throughput / MB/s"].append(
                temp["Mean Throughput / MB/s"])
    ...
    sns.lineplot(x="Packet Loss / %", y="Average Throughput / MB/s",
                 hue="Simulation", data=df, estimator=np.mean)
    plt.title("TCP with and without Rely \n" + f'Packets = {temp["Transfers"]}\n' + (
